...
export class RegistrationService {
  ...
  /**
   * The LEAN pledge certificate for this registration.
   *
   * Generated by the server each time it is asked for and never stored, so the
   * certificate always carries the details the registration holds now.
   */
  pledgeCertificate(token: string): Observable<Blob> {
    return this.http.get(`${this.base}/${token}/pledge`, { responseType: 'blob' });
  }
